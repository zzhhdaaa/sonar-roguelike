...
sealed class AdamMilVisibility : Visibility
{
    ...
    public override void Compute(Vector3Int origin, int rangeLimit, List<Vector3Int> fieldOfView)
    {
        ...
        for (uint octant = 0; octant < 8; octant++)
            Compute(octant, origin, rangeLimit, 1, new Slope(1, 1), new Slope(0, 1), fieldOfView);
    }
    ...
    struct Slope // represents the slope Y/X as a rational number
    {
        ...
        public bool Greater(uint y, uint x)
        {
            return Y * x > X * y;
        } // this > y/x
        public bool GreaterOrEqual(uint y, uint x)
        {
            return Y * x >= X * y;
        } // this >= y/x
        public bool Less(uint y, uint x)
        {
            return Y * x < X * y;
        } // this < y/x
        ...
        for (; x <= (uint)rangeLimit; x++) // (x <= (uint)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
        {
            ...
            for (uint y = topY; (int)y >= (int)bottomY; y--) // use a signed comparison because y can wrap around when decremented
            {
                if (rangeLimit < 0 || (Mathf.Abs(x) + Mathf.Abs(y)) <= rangeLimit) // skip the tile if it's out of visual range
                {
                    ...
                    if (isVisible)
                        SetVisible(x, y, octant, origin, fieldOfView);
                            ...
                            {                  // so adjust the bottom vector upward and continue processing it in the next column
                                ...
                                {                       // created by adjusting the bottom is only valid if that's the case
                                    ...
                                    if (y == bottomY)
                                    {
                                        ...
                                        Compute(octant, origin, rangeLimit, x + 1, top, new Slope(ny, nx), fieldOfView);
                                }
